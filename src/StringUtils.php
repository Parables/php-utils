...
trait StringUtils
{
    ...
    public static function capitalize(string $string = ''): string
    {
        return self::normalizeWhitespace(
            string: implode(
                ...
                array: preg_split(
                    pattern: '/(?=[A-Z])/',
                    subject: self::pascalCase(string: $string)
                )
            )
        );
    }
    ...
    {
        $string = self::slugify(string: $string, toLowerCase: false);  // implode(separator: '_', array: words(string: $string));
        $string = preg_replace('/(?<!\b)[A-Z]/', ' $0', $string);
        $string = str_replace(search: ' ', replace: '_', subject: $string);
